"""Entrypoint: LLM rerank per eos_id.
...
def main() -> None:
    ...
    for i, eos_id in enumerate(eos_ids, start=1):
        ...
        out_path = OUTPUT_DIR / f"{eos_id}_rerank.json"
        out_path.write_text(
            json.dumps([row.model_dump() for row in result], indent=2, ensure_ascii=False),
            encoding="utf-8",
        )
        logger.info(
            "Saved rerank: %d attrs -> %s",
            len(result),
            out_path,
        )
